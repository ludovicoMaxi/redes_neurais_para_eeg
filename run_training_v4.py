...
class CnnTranformerEncodeFeatures(CnnLstmFeatures):
    ...
    def get_model(self, datas_for_training):
        ...
        #transformer_encoder = TransformerEncoder(num_heads=8, intermediate_dim=128, dropout=0.1)
        # Adicionando o TransformerEncoder no lugar do LSTM
        #hidden5 = transformer_encoder(hidden4)

        gp = tf.keras.layers.GlobalAveragePooling1D()(hidden4)
...
process_training_ai(file_path='./data/patients.xlsx', ai_type=AiType.DEATH_PROGNOSTIC_STATISTIC_FEATURE, resample_data_same_size=True, group_fold=False)
