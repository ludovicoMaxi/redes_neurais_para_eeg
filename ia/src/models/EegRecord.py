...
class EegRecord:
    ...
    default_variables_description.append(Variable(
        'fa', '(escalar): frequencia de amostragem do exame'))
    default_variables_description.append(Variable(
    ...
    default_variables_description.append(
        Variable('fa', '(escalar): frequencia de amostragem do exame em Hz'))
    default_variables_description.append(Variable(
        'T', '(escalar): tempo de amostragrem que eh igual 1/fa, periodo de amostragem (baseado na taxa de amostragem) em segundos'))
    ...
    def __init__(self, name, channel_list, fa, xn, freq_max=100, variables_description=default_variables_description, customize=True):
        """Construtor
            Args:
                ...
                channel_list (vetor de namedtuple('Channel', ['name', 'type']) ): canais utilizados
                    ...
                    type pode ser (EEG, EKG, FOTO)
                    Possui  correspondencia direta linha a linha com "xn"
                fa (escalar): frequencia de amostragem do exame em Hz
                xn (ndarray numpy de escalares): cada linha desta matriz representa os valores de coleta de cada canal
                    Matriz de LxN
                    Onde:
                        L = quantidade de canais EEG do registro  [comumente L=21]
    ...
    def __repr__(self):
        return "EegRecord={{name: {}, fa: {}}}".format(self.__name, self.__fa)
    ...
    def T(self):
        """
            T (escalar): tempo de amostragrem que eh igual 1/fa
        """
        return 1/self.__fa
    ...
    def read_file_plg(path_file, customize=True):
        ...
        with open(path_file, "rb") as file_plg:
            ...
            for i in range(amount_channels):
                ...
                channel_name = header_channel[11:16].decode('UTF-8')
        ...
        return EegRecord(name, channel_list, fa, xn, customize=customize)
    ...
    def epoch_separator(self, start_time_epoch, windown_length):
        """epoch_separator
            ...
            Args:
                ...
                window_length (escalar): tamanho da janela em milissegundos
        """
        start_time = int((start_time_epoch/1000) * self.__fa)
        end_time = start_time + int(windown_length * self.__fa / 1000)
        ...
        self.__epoch_list.append(
            Epoch(start_time_epoch, windown_length, self.__fa, xn_epoch, self.__frequence_band_list)
        )
    ...
    def update_frequence_band_list_and_freq_max_according_with_freqMax_and_fa(self, freq_max):
        ...
        """
        if freq_max > (self.__fa / 2):
            freq_max = int(self.__fa / 2)
    ...
    def proccess_records(abs_path_csv_file):
        ...
        with open(abs_path_csv_file, mode='r') as csv_file:
            ...
            for idx_line, row in enumerate(csv_reader):
                ...
                file_name_plg = row[name_column_file]+".PLG"
                ...
                epochs_amount = int(row["Qtd Epocas"])
                start_time_epoch_list = []

                for i in range(epochs_amount):
                    column_epoch_x = ("Ep" + str(i+1))
                    time_epoch = row[column_epoch_x]
                    time_minute = int(time_epoch.split(':')[0])
                    time_second = int(time_epoch.split(':')[1])

                    start_time_epoch_list.append(
                        (time_minute*60 + time_second)*1000)
                ...
                start_time_step = time.time()
                LOG.info('Inicio Validação de canias ruidosos...')
                ...
                LOG.info(f'Fim Validação de canias ruidosos. ({time.time() - start_time_step} segundos)')
                ...
                is_for_compute_coerencia = 'COERENCIA' in compute_quantifiers_list
