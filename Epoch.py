...
class Epoch:
    ...
    def __init__(self, start_at, window_length, fa, xn, frequence_band_list):
        """Construtor
        ...
        self.__fa = fa
        self.__xn = xn
        #ANALISAR# validar tamanho da janela com fa e xn
    ...
    @staticmethod
    def auto_correlation(xn, fa, tau_max):
        """auto_correlation
            ...
            Args:
                ...
                tau_max (escalar): representa o comprimeto do vetor Rx (quantidade de amostras).
                    ...
                    (Em 2015-2017 para sinais EEG recomenda-se usar igual a 100) ou fix(1.67*fa)
        ...
        for i in range(amount_lines):
            ...
            for tau in range(1, tau_max):
                ...
                #rx_channel[tau] = rx_channel[tau]/(amount_columns - tau)*(1/fa)
                ...
                rx_channel[tau] = rx_channel[tau]/(amount_columns - tau)*(fa)
    ...
    def dft_matriz_v3(rx, fa):
        ...
        t = np.array([i/fa for i in range(amount_columns)])
        f = [i*fa/amount_columns for i in range(int(amount_columns/2) + 1)]
        ...
        for r in range(amount_lines):
            ...
            for k in range(len(f)):
                ...
                xrf = np.trapz(Cx, dx=1/fa)[0, 0]
                xif = np.trapz(Sx, dx=1/fa)[0, 0]
    ...
    def compute_meanZero_powerSignal_RX_TX_Ftest_XM_XF_xmFiltered_XmNorm_xkSignal_bands(self, tau_max, freq_max, band_frequence_list):
        ...
        rx = self.auto_correlation(deviation_mean_xn, self.__fa, tau_max)

        direct_fourier_transform = self.dft_matriz_v3(rx, self.__fa)
    ...
    def compute_coherence_side_left_with_right(self, channel_list):
        ...
        for i in range(n_left):
            ...
            frequences, coherence = signal.coherence(mean_zero_left, mean_zero_rigth, fs=self.__fa, nfft=nfft, window=signal.get_window('hamming', L, fftbins=False), noverlap=int(L//2))
            # coherence(x,y,'hann',noverlap=3,nperseg=6,fs=1,detrend=False)
            # coherence2 = mlab.cohere(mean_zero_left, mean_zero_rigth, window=np.hamming(L), NFFT=nfft, noverlap=int(L//2), Fs=self.__fa)
