...
class ModelAi(ABC):
    ...
    def read_pkl_file(self, file_path, df_info_patients, resample_data_same_size=False):
        ...
        shape_channels = np.shape(eeg_data_trunc[0])
        #igualar a quantidade de atributos/amostras
        fs_epoch = SFREQ if resample_data_same_size else eeg_record.fa
        if( resample_data_same_size ):
            sample: npt.NDArray[np.object_] = np.zeros(shape=(5), dtype=np.object_)
            sample[0] = eeg_data_trunc
    ...
    def extract_common_spatial_pattern(self, datas, labels):
        ...
        if(self.get_analysis_type() == AnalysisType.ETIOLOGY):
            label_encoder = LabelEncoder()
            labels = label_encoder.fit_transform(labels)
            

        # Instanciando o CSP
        csp = CSP(n_components=amount_components, reg=None, log=True, norm_trace=False)

        # Ajustando o CSP aos dados
        csp.fit(datas, labels)
        print('###########################################')
        print(labels)
        print('###########################################')
    ...
    def transform_data_common_spatial_pattern(self, df_dict):
        ...
        patients_data_features = self.extract_common_spatial_pattern(features, y)
...
process_training_ai(file_path='./data/patients.xlsx', ai_type=AiType.DEATH_PROGNOSTIC_TRANFORMER_COHERENCE_FEATURE, resample_data_same_size=True, group_fold=False)
